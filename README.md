...
*   **🔒 비밀번호 암호화**: 모든 계정 비밀번호는 **SHA-256 알고리즘과 고유 솔트(Salt)**를 사용하여 단방향 해싱 저장됩니다.
*   **🛡️ 브루트 포스 방지**: 로그인 3회 실패 시 해당 연결을 즉시 차단합니다.
*   **🚦 도스(DoS) 방여**: 서버 전체 동시 접속 50명, IP당 5개로 제한하여 서버 자원 고갈을 방지합니다.
...
│   ├── app.py          # 메인 프레임워크
│   ├── core/           # 핵심 엔진 (설정, 보안, 유틸)
│   └── gui/tabs/       # UI 컴포넌트 (Server, Client)
├── config/             # 데이터 저장소 (JSON, SSL 인증서)
├── run.bat             # 윈도우 원클릭 실행
└── README.md           # 공식 문서
```
