...
1. **설치 (권장)**: `run.bat` 파일을 실행하세요. 필요한 라이브러리(`pyftpdlib`, `cryptography` 등)를 자동으로 설치하고 실행합니다.
    > **주의**: 릴리즈된 `.exe` 파일은 **파일 쓰기 권한 문제**와 **백신 프로그램 오탐지(False Positive)** 가능성이 있으므로, 소스 코드를 통해 실행하는 `run.bat` 사용을 강력히 권장합니다.
