...
*   **🌓 다크 테마 로그 시스템**: 실시간 활동 로그를 다크 모드로 구성하여 장시간 가동 시에도 눈의 피로를 최소화합니다.
*   **📐 지능형 가변 레이아웃**: 핵심 설정 영역과 로그 영역의 가중치를 최적화하여, 포트 및 IP 정보가 누락 없이 한눈에 들어오도록 설계되었습니다.
*   **🌐 네트워크 상태 대시보드**: 내부 및 공인 IP 상태를 시각적으로 분리하여 로컬망과 외부망 연결성을 즉각적으로 파악할 수 있습니다.
*   **📁 인라인 계정 관리**: 별도의 창 전환 없이 한 화면에서 계정 생성, 권한 부여, 경로 설정을 즉시 처리하는 고효율 인터페이스를 제공합니다.
