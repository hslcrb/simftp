...
*   **📡 PowerShell 기반 고신뢰도 IP 조회**: 시스템 네트워크 스택을 가장 정직하게 사용하는 `PowerShell (Invoke-RestMethod)` 엔진을 도입하여, 일반적인 라이브러리나 `curl`이 감지하지 못하는 복잡한 프록시 환경에서도 실제 공인 IP(`.199`)를 정확히 추적합니다.
